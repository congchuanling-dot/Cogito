...
export default function ArticleCard({ article }: { article: Article }) {
  ...
  return (
    <Link to={`/article/${article.slug}`} className="no-underline group block">
      <article className="card-glow bg-geek-surface border border-geek-border rounded-lg overflow-hidden">
        <TerminalBar title={`${article.slug}.md — ${date}`} />
        ...
        <div className="p-5">
          ...
          <h2 className="text-base font-semibold text-geek-text-h group-hover:text-geek-accent group-hover:text-glow transition-all mb-2 font-mono">
            {article.title}
          </h2>
          {article.excerpt && (
